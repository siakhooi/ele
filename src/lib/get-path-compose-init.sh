...
file="$ele_home/$compose_filename/compose-init.yaml"
if [[ -f $file ]]; then
	echo "$file"
	exit 0
fi
file="$ele_home/$compose_filename/compose-init.yml"
if [[ -f $file ]]; then
	echo "$file"
	exit 0
fi
file="$ele_home/$compose_filename/docker-compose-init.yaml"
if [[ -f $file ]]; then
	echo "$file"
	exit 0
fi
file="$ele_home/$compose_filename/docker-compose-init.yml"
if [[ -f $file ]]; then
	echo "$file"
	exit 0
fi
